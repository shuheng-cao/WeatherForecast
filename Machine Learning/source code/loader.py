...
def load_data(size, cached = True):
    if cached and str(size) in os.listdir("cache/"):
        print("Loading data from cache ...")
        return pickle.load(open(f"cache/{size}", "rb"))

    res = []
    data_set, rain_set, snow_set =  set([int(filename[:-4]) for filename in os.listdir("../training data/past data/") if filename[-4:] == ".txt"]), \
                                    set([int(filename[:-4]) for filename in os.listdir("../training data/processed image data/rain") if filename[-4:] == ".jpg"]), \
                                    set([int(filename[:-4]) for filename in os.listdir("../training data/processed image data/snow") if filename[-4:] == ".jpg"])
    # we may loss or have some bad data for certain days
    valid_set = data_set & rain_set & snow_set
    fail = 0
    for time in valid_set:
        if len(res) == size:
            with open(f'cache/{size}', 'wb') as f:
                pickle.dump(res, f)
            return res
        ...
        if next_hour in valid_set:
            try:
                rain_img = Image.open(f"../training data/processed image data/rain/{time}.jpg")
                snow_img = Image.open(f"../training data/processed image data/snow/{time}.jpg")
                ...
                cur_data = pickle.load(open(f"../training data/past data/{time}.txt", "rb"))["current"]
                nex_data = pickle.load(open(f"../training data/past data/{next_hour}.txt", "rb"))["current"]
